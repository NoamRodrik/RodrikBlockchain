...
void SHA256::CalculateDigest(const unsigned char* data, size_t numBytes)
{
    if (buffer_size_ > 0)
    {
        while (numBytes > 0 && buffer_size_ < BLOCK_SIZE)
        {
    ...
    while (numBytes > 0)
    {
        if (this->buffer_size_ > sizeof(buffer_))
        {
            printf("Got an invalid buffer size");
            abort();
        }
...
void SHA256::ProcessBuffer()
{
    ...
    size_t padded_length{buffer_size_ * 8 + 1};
    ...
    size_t i{};
    for (i = buffer_size_ + 1; i < BLOCK_SIZE; i++)
    {
        buffer_[i] = 0;
    }
    ...
    uint64_t test_bits = 8 * (bytes_ + buffer_size_);

    // find right position
    unsigned char* append_len{padded_length < BLOCK_SIZE ? buffer_ + padded_length : extra + padded_length - BLOCK_SIZE};

    // must be big endian
    *append_len++ = (unsigned char)((test_bits >> 56) & 0xFF);
    *append_len++ = (unsigned char)((test_bits >> 48) & 0xFF);
    *append_len++ = (unsigned char)((test_bits >> 40) & 0xFF);
    *append_len++ = (unsigned char)((test_bits >> 32) & 0xFF);
    *append_len++ = (unsigned char)((test_bits >> 24) & 0xFF);
    *append_len++ = (unsigned char)((test_bits >> 16) & 0xFF);
    *append_len++ = (unsigned char)((test_bits >> 8) & 0xFF);
    *append_len = (unsigned char)(test_bits & 0xFF);
...
void SHA256::FillHash(std::array<unsigned char, 2 * HASH_BYTES>& hash_container)
{
    // save old hash if buffer is partially filled
    uint32_t old_hash[HASH_VALUES]{};
    ...
    std::array<unsigned char, HASH_BYTES> hash{};
    size_t hash_index{0};
    for (int i = 0; i < HASH_VALUES; i++)
    {
        hash[hash_index++] = (hash_[i] >> 24) & 0xFF;
        hash[hash_index++] = (hash_[i] >> 16) & 0xFF;
        hash[hash_index++] = (hash_[i] >> 8) & 0xFF;
        hash[hash_index++] = hash_[i] & 0xFF;
    ...
    {
        static constexpr const char DEC2HEX[16 + 1] = "0123456789abcdef";
        hash_container[2 * i] = DEC2HEX[(hash[i] >> 4) & 15];
        hash_container[2 * i + 1] = DEC2HEX[hash[i] & 15];
    }
}